...
function Initialize-MainWindow {
    ...
    $form.Size = New-Object System.Drawing.Size($script:Config.ui.windowWidth, $script:Config.ui.windowHeight)
...
function Add-ProgressBar {
    ...
    $centerY = ($Form.Height - $script:Config.ui.progressBarHeight) / 2
    $progressBar.Location = New-Object System.Drawing.Point(20, $centerY)
    $progressBar.Size = New-Object System.Drawing.Size(440, $script:Config.ui.progressBarHeight)
...
function Add-StatusLabel {
    ...
    $centerY = ($Form.Height / 2) - 30
    $label.Location = New-Object System.Drawing.Point(20, $centerY)
    $label.Size = New-Object System.Drawing.Size(440, 25)
    ...
    try {
        ...
        if ($drives.Count -eq 1) {
            ...
            do {
                ...
                if ($selectedDrive -notlike "*:") { $selectedDrive += ":" }
...
function Invoke-Step5 { 
    ...
    try {
        ...
        if ($localFile) {
            ...
            Copy-Item -Path $localFile -Destination (Join-Path $targetPath "7zr.exe") -Force
        ...
        foreach ($depKey in $dependencies.packages.PSObject.Properties.Name) {
            if ($depKey -eq "7zr") { continue }  # Already processed
...
function Invoke-Step6 { 
    ...
    try {
        ...
        $script:DatabasePort = Find-AvailablePort -StartPort $dbConfig.portRange.start -EndPort $dbConfig.portRange.end
        
        if (-not $script:DatabasePort) {
...
"@
        ...
        Write-Log -Message "Executing application setup script" -Level "INFO"
...
function Register-WindowsService {
    ...
    try {
        ...
        $scArgs = @(
            "failure", $ServiceConfig.name,
            "reset=", $ServiceConfig.resetInterval,
            "actions=", "restart/$($ServiceConfig.restartDelay)/restart/$($ServiceConfig.restartDelay * 2)/restart/$($ServiceConfig.restartDelay * 3)"
        )
...
function Main {
    try {
        ...
        $buttonY = ($script:MainForm.Height / 2) + 50
        $startButton.Location = New-Object System.Drawing.Point(200, $buttonY)
